...
export function usePushSubscribe(): UsePushSubscribeType {
  ...
  const fetchChainId = () => {
    const chainId = "5";
    const env = "staging";
    const channel = PushSettings.STAGING_CHANNEL_ADDRESS;
  ...
  const isUserSubscribed = useCallback(async () => {
    ...
    const addresses = userSubscriptions.map(({ channel }: { channel: string }) => channel);
  ...
  async function _signTypedData(domain: unknown, types: unknown, value: unknown) {
    let params: SignerType;
    // if they are all objects
    if (typeof domain === "object" && typeof types === "object" && typeof value === "object") {
      params = {
        domain: { ...domain },
        types: { ...types },
        value: { ...value },
